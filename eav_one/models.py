...
class Attribute(models.Model):
    ...
    enum_group = models.ForeignKey(EnumGroup, verbose_name=_(u"choice group_one"),
                                   blank=True, null=True)

    type = models.CharField(_(u"type"), max_length=20, blank=True, null=True)

    form_step = models.ForeignKey("questionary_one.FormStep")
    ...
    def clean(self):
        ...
        if self.datatype not in [self.TYPE_RADIO,self.TYPE_ENUM] and self.enum_group:
            raise ValidationError(_(
...
class Value(models.Model):
    ...
    entity_ct = models.ForeignKey(ContentType, related_name='value_entities_one')
    entity_id = models.IntegerField()
    entity = fields.GenericForeignKey(ct_field='entity_ct',
                                       fk_field='entity_id')
    ...
    value_enum = models.ForeignKey(EnumValue, blank=True, null=True,
                                   related_name='eav_values_one')

    generic_value_id = models.IntegerField(blank=True, null=True)
    generic_value_ct = models.ForeignKey(ContentType, blank=True, null=True,
                                         related_name='value_values_one')
    value_object = fields.GenericForeignKey(ct_field='generic_value_ct',
